...
def get_predictions() -> dict:
    ...
    query = """
        SELECT DISTINCT ON (team) team, off_eff, def_eff, power_rank 
        FROM (
            SELECT home_team as team, home_off_eff as off_eff, home_def_eff as def_eff, home_power_rank as power_rank, game_date 
            FROM model_training_data
            UNION ALL
            SELECT away_team as team, away_off_eff as off_eff, away_def_eff as def_eff, away_power_rank as power_rank, game_date 
            FROM model_training_data
        ) t 
    ...
    # Mapping to exact names in DB:
    # Pelcians -> no, Jazz -> utah, PHI -> phi, Heat -> mia, Magic -> orl, Rockets -> hou, Pacers -> ind, Hornets -> cha
    spread_slate = [
        {"h": "utah", "a": "no", "l": "nba", "v": 4.5},      
        {"h": "phi", "a": "mia", "l": "nba", "v": -1.5},       
        {"h": "orl", "a": "hou", "l": "nba", "v": -2.5},      
        {"h": "purdue", "a": "michigan st", "l": "ncaab", "v": 9.5}, 
        {"h": "ind", "a": "cha", "l": "nba", "v": 6.0}     
    ]
    
    nba_res: List[List[Any]] = [["Matchup", "Vegas Line", "Model Pred", "Edge"]]
    ncaab_res: List[List[Any]] = [["Matchup", "Vegas Line", "Model Pred", "Edge"]]
    prop_res: List[List[Any]] = [["Player", "Prop Line", "Model Pred", "Edge"]] 

    for g in spread_slate:
        h_s, a_s = stats.get(g['h']), stats.get(g['a'])
        if not h_s or not a_s:
            logger.warning(f"Missing stats for {g['a']} ({'Found' if a_s else 'Missing'}) or {g['h']} ({'Found' if h_s else 'Missing'})")
            continue
            
        model = xgb.XGBRegressor()
        model.load_model(f"{g['l']}_model.json")
        
        feats = pd.DataFrame([[
            h_s['off_eff'], h_s['def_eff'], 
            a_s['off_eff'], a_s['def_eff'], 
            h_s['power_rank'], a_s['power_rank']
...
def upload_to_sheets(data_dict: dict):
    try:
        ...
        logger.success("Dashboard successfully updated!")
